  ...
  return {
    ...
    async resolve(context: ResolverContext): Promise<Impulse[]> {
      ...
      const prompt = [
        ...
        `  body: { vesselId: process.env.VESSEL_ID ?? '${vesselShape}-vessel', vesselName: '${vesselShape}-vessel', version, shapes: ['${vesselShape}'], endpoint: vesselEndpoint, resolve_endpoint: \`\${vesselEndpoint}/v2/impulses/resolve\`, auth_scheme: 'ApiKey' }`,
        "  NOTE: discovery-vessel's RegisterRequest schema uses vesselId+vesselName (not id+name).",
        ...
        "  body: { vesselId: process.env.VESSEL_ID ?? '<shape>-vessel', shapes: ['<shape>'] }",
        "  NOTE: heartbeat also uses vesselId (not id).",
